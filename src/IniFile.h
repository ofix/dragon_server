...
class IniFileBase : public std::map<std::string, IniSectionBase<Comparator>, Comparator> {
   ...
   public:
    ...
    void setCommentChar(const char comment) { commentPrefixes_ = {std::string(1, comment)}; }
    ...
    void allowOverwriteDuplicateFields(bool allowed) { overwriteDuplicateFields_ = allowed; }
